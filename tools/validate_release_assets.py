"""Static release-asset validation for the Grounding DINO detection DIMER pipeline.
...
EXPECTED_OUTPUTS = (
    ...
    'outputs/grounding_dino_detection_detections.csv',
    'outputs/grounding_dino_detection_annotated.png',
)
# Profile-specific code the notebook must exercise through the repository public API.
CODE_MARKERS = (
    'from grounding_dino_detection_pipeline import MAX_IMAGE_SIDE, MAX_PROMPT_CHARS, '
    'MAX_PROMPTS, MAX_TEXT_TOKENS, MIN_IMAGE_SIDE, format_prompts',
    'from grounding_dino_detection_pipeline import MODEL_ID, MODEL_KEY, MODEL_REVISION, '
    'GroundingDINOPipeline, box_iou, stage_missing_files, verify_snapshot',
    ...
    'box_threshold = 0.4',
    'text_threshold = 0.3',
    'prompt_text = format_prompts(prompts)',
...
MARKDOWN_MARKERS = (
    ...
    'The detection thresholds are **caller-owned request parameters**, not pipeline constants',
    '**sigmoid grounding score, not a calibrated probability**',
    '**ordered by descending score**',
    'No detection metric is reported: mean average precision needs a labelled box set',
    'as a **sanity check** that the geometry round-trips',
    'instance or semantic segmentation (see the sibling SAM 2 pipeline), tracking, OCR, captioning',
)
